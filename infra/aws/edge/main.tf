...
resource "terraform_data" "alb_certificate_required" {
  input = var.alb_certificate_arn

  lifecycle {
    precondition {
      condition     = var.environment == "local" || var.alb_certificate_arn != null
      error_message = "alb_certificate_arn is required for non-local AWS environments so Core traffic is served only through HTTPS."
    }
  }
}
...
resource "aws_lb_listener" "http" {
  ...
  default_action {
    type = var.alb_certificate_arn == null ? "fixed-response" : "redirect"
    ...
    dynamic "fixed_response" {
      ...
      content {
        ...
        message_body = "HTTPS is required for TaskMind Core API access."
        status_code  = "426"
      }
    }
...
resource "aws_wafv2_web_acl_association" "core" {
  resource_arn = aws_lb.core.arn
  web_acl_arn  = aws_wafv2_web_acl.core.arn
}

resource "aws_cloudfront_origin_access_control" "spa" {
  name                              = "${local.name}-spa"
  description                       = "OAC for TaskMind SPA artifacts"

...
variable "environment" {
  ...
  validation {
    ...
    error_message = "The production root may only be used with environment = production."
  }
}


variable "github_environment" {
  description = "GitHub Environment name used by the infrastructure plan workflow. Keep this environment protected with required reviewers and environment-scoped variables/secrets."
  type        = string
  default     = "production"

  validation {
    condition     = var.github_environment == "production"
    error_message = "Production infrastructure plans must run from the protected production GitHub Environment."
  }
}

...
data "aws_iam_policy_document" "core_policy" {
  statement {
...
resource "aws_iam_role_policy" "task" {
  for_each = {
    core  = data.aws_iam_policy_document.core_policy.json
    relay = data.aws_iam_policy_document.relay_policy.json
    nova  = data.aws_iam_policy_document.nova_policy.json
  }
...
resource "aws_ecs_task_definition" "service" {
  ...
  lifecycle {
    precondition {
      condition = alltrue([
        for secret in lookup(var.service_secrets, each.key, []) : anytrue([
          for arn in local.service_secret_arns[each.key] : secret.valueFrom == arn || startswith(secret.valueFrom, "${arn}:")
        ])
      ])
      error_message = "Every service_secrets entry must use a valueFrom ARN listed in the matching <service>_secret_arns variable."
    }
  }
}
...
resource "aws_ecs_service" "service" {
  ...
  dynamic "load_balancer" {
    for_each = each.key == "core" ? [1] : []

    content {
      target_group_arn = var.core_target_group_arn
      container_name   = "core"
      container_port   = 8080
    }
  }

    ...
    public void project(DomainEvent event) {
        ...
        try {
            jdbcTemplate.update(
                    ...
                    taskId, event.scope().userId(), event.scope().projectId(), title, status, event.occurredAt());
        } catch (DuplicateKeyException ex) {
            jdbcTemplate.update(
                    "update analytics.task_projection set user_id=?, project_id=?, title=?, status=?, updated_at=? where task_id=?",
                    event.scope().userId(), event.scope().projectId(), title, status, event.occurredAt(), taskId);
        }

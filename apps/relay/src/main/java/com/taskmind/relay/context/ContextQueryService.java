...
public class ContextQueryService {
    ...
    public List<Map<String, Object>> userTasks(UUID userId) {
        return jdbcTemplate.queryForList(
                "select task_id, project_id, title, status, updated_at from analytics.task_projection where user_id=? order by updated_at desc limit 50",
                userId);
    }

    public List<Map<String, Object>> projectTasks(UUID projectId) {
        return jdbcTemplate.queryForList(
                "select task_id, user_id, title, status, updated_at from analytics.task_projection where project_id=? order by updated_at desc limit 50",
                projectId);
    }

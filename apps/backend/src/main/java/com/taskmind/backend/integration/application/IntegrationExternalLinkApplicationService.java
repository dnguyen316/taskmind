...
public class IntegrationExternalLinkApplicationService {
    ...
    public IntegrationExternalLinkApplicationService(IntegrationExternalLinkRepository links) {
        this.links = links;
    }

    public Optional<IntegrationExternalLink> findExistingExternalIssue(
            IntegrationProvider provider, String externalId, String externalKey) {
        return links.findByProviderAndExternalTypeAndExternalIdentity(provider, "ISSUE", externalId, externalKey);
    }

    public IntegrationExternalLink record(
            UUID taskId,
            UUID projectId,
            ...
            String direction,
            String metadataJson) {
        return links.findByTaskIdAndProvider(taskId, provider).orElseGet(() -> {
            Instant now = Instant.now();
            return links.save(new IntegrationExternalLink(
                    UUID.randomUUID(),
                    null,
                    taskId,
                    projectId,

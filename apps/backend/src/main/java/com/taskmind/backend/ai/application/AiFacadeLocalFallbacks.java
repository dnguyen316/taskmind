...
public class AiFacadeLocalFallbacks {
    public CaptureResult capture(String text) {
        ...
        return new CaptureResult(drafts, drafts.isEmpty() ? "Could you share at least one action-oriented task?" : null, null, false);
    }
    ...
    public GoalBreakdownResult goalBreakdown(UUID goalId, OffsetDateTime deadline, Integer weeklyAvailabilityMinutes) {
        ...
        return new GoalBreakdownResult(
                ...
                List.of("Review generated dates against team capacity (" + (weeklyAvailabilityMinutes == null ? "unknown" : weeklyAvailabilityMinutes) + " minutes/week)."), null, false);
    }

    public WeeklyReviewResult weeklyReview(UUID userId) {
        return new WeeklyReviewResult(userId, "You maintained steady execution and have a clear next planning loop.", List.of("Some work may need tighter estimates."), List.of("Reserve buffer before committing each day.", "Review blockers before rescheduling."), List.of("Stabilize the highest-priority active project."), null, false);
    }

    public ProjectBriefResult projectBrief(UUID projectId, String name, String description) {
        return new ProjectBriefResult(projectId, "Project " + safe(name) + " is ready for focused execution.", List.of("Clarify deliverables", "Sequence the next milestone"), List.of("Unreviewed assumptions may affect scope."), List.of("Confirm owner", "Create the next three tasks"), null, false);
    }
    ...
    public DurationEstimateResult durationEstimate(String title, String description) {
        ...
        return new DurationEstimateResult(minutes, "Estimated locally from task complexity and text length.", 0.65d, null, false);
    }

    public RationalePhraseResult rationalePhrase(String title, String context) {
        return new RationalePhraseResult("Scheduled to protect focus for " + safe(title) + ".", null, false);
    }

    public DashboardInsightsResult dashboardInsights(UUID userId) {
        return new DashboardInsightsResult(userId, "Your dashboard is ready for review.", List.of("Prioritize active work with near-term due dates."), List.of("Keep daily plans below available capacity."), null, false);
    }

    public DescribeTaskResult describe(String title, String notes) {
        String suffix = notes == null || notes.isBlank() ? "" : " Notes: " + notes.trim();
        return new DescribeTaskResult("Complete: " + title.trim() + "." + suffix, "Generated locally from the task title and notes.", null, false);
    }
    ...
    public DescribeTaskAutocompleteResult autocomplete(String text) {
        ...
        return new DescribeTaskAutocompleteResult(List.of(base + " with clear acceptance criteria.", base + " and identify blockers."), null, false);
    }

    public TranslateTaskResult translate(String text, String targetLanguage) {
        return new TranslateTaskResult("[" + targetLanguage + "] " + text.trim(), targetLanguage, null, false);
    }

...
public class IntegrationPublishApplicationService {
    private final IntegrationProjectLinkApplicationService links; private final IntegrationPublishRecordRepository records; private final IntegrationExternalLinkApplicationService externalLinks; private final TaskApplicationService tasks; private final JiraCloudClient jira; private final WikiClient wiki;
    public IntegrationPublishApplicationService(IntegrationProjectLinkApplicationService links, IntegrationPublishRecordRepository records, IntegrationExternalLinkApplicationService externalLinks, TaskApplicationService tasks, JiraCloudClient jira, WikiClient wiki) { this.links = links; this.records = records; this.externalLinks = externalLinks; this.tasks = tasks; this.jira = jira; this.wiki = wiki; }
    @Transactional
    public IntegrationPublishRecord publish(AuthenticatedUser actor, UUID taskId, UUID projectLinkId) {
        IntegrationProjectLink link = links.requireAccessible(actor, projectLinkId); Task task = tasks.findById(actor, taskId).orElseThrow(() -> new IllegalArgumentException("Task not found"));
        return records.findByTaskIdAndProvider(taskId, link.provider()).orElseGet(() -> {
            String id, key, url, type;
            if (link.provider() == IntegrationProvider.JIRA) { JiraCloudClient.PublishedIssue p = jira.publish(link.externalProjectKey(), task.title(), task.taskType().name()); id = p.id(); key = p.key(); url = p.url(); type = "ISSUE"; }
            else if (link.provider() == IntegrationProvider.WIKI) { WikiClient.PublishedPage p = wiki.publish(link.externalProjectKey(), task.title()); id = p.id(); key = p.key(); url = p.url(); type = "PAGE"; }

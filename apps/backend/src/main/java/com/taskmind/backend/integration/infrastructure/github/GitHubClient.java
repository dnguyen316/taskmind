...
public class GitHubClient {
    ...
    public RepositoryMetadata getRepository(String baseUrl, String accessToken, String owner, String repo) {
        ...
        return new RepositoryMetadata(
                node.path("node_id").asText(node.path("id").asText()),
                node.path("owner").path("login").asText(owner),
                node.path("name").asText(repo),
                node.path("full_name").asText(owner + "/" + repo),
                node.path("default_branch").asText(null),
                node.path("private").asBoolean(false),
                node.path("html_url").asText(null),
                node.path("installation").path("id").asText(null),
                node.path("owner").path("id").asText(null));
    }
    ...
    public List<Comment> listIssueComments(
            String baseUrl, String accessToken, String owner, String repo, int issueNumber) {
    ...
    public Comment createIssueComment(
            String baseUrl, String accessToken, String owner, String repo, int issueNumber, String body) {
        try {
            JsonNode node = restClient
                    .post()
                    .uri(apiBaseUrl(baseUrl), uri -> uri.path("/repos/{owner}/{repo}/issues/{issueNumber}/comments")
    ...
    public Ref createBranchRef(
            String baseUrl, String accessToken, String owner, String repo, String branchName, String fromSha) {
        try {
            JsonNode node = restClient
                    .post()
                    .uri(apiBaseUrl(baseUrl), uri -> uri.path("/repos/{owner}/{repo}/git/refs").build(owner, repo))
                    .headers(headers -> headers.setBearerAuth(accessToken))
                    .body(new RefRequest("refs/heads/" + branchName, fromSha))
                    .retrieve()
                    .body(JsonNode.class);
    ...
    public PullRequest createPullRequest(
        ...
        try {
            JsonNode node = restClient
                    .post()
                    .uri(apiBaseUrl(baseUrl), uri -> uri.path("/repos/{owner}/{repo}/pulls").build(owner, repo))
                    .headers(headers -> headers.setBearerAuth(accessToken))
                    .body(new PullRequestRequest(title, head, base, body))
                    .retrieve()
                    .body(JsonNode.class);
    ...
    public PullRequestStatus getPullRequestStatus(
            String baseUrl, String accessToken, String owner, String repo, int pullNumber) {
        JsonNode pullRequestNode = get(
                apiBaseUrl(baseUrl), accessToken, "/repos/{owner}/{repo}/pulls/{pullNumber}", owner, repo, pullNumber);
        return new PullRequestStatus(
                pullRequestNode.path("node_id").asText(pullRequestNode.path("id").asText()),
                pullRequestNode.path("number").asInt(pullNumber),
                pullRequestNode.path("state").asText(),
                pullRequestNode.path("mergeable").isMissingNode()
                        ? null
                        : pullRequestNode.path("mergeable").asBoolean(),
                pullRequestNode.path("merged").asBoolean(false),
                pullRequestNode.path("head").path("sha").asText(null));
    }
    ...
    public List<ExternalIssue> importIssues(String baseUrl, String accessToken, String repository, int limit) {
        try {
            ...
            JsonNode response = restClient
                    .get()
                    .uri(apiBaseUrl(baseUrl), uri -> uri.path("/repos/{owner}/{repo}/issues")
    ...
    private JsonNode get(String apiBaseUrl, String accessToken, String path, Object... args) {
        try {
            return restClient
                    .get()
                    .uri(apiBaseUrl, uri -> uri.path(path).build(args))
                    .headers(headers -> headers.setBearerAuth(accessToken))
                    .retrieve()
                    .body(JsonNode.class);
        } catch (RestClientException exception) {
    ...
    private String apiBaseUrl(String baseUrl) {
        return baseUrl == null || baseUrl.isBlank() ? "https://api.github.com" : baseUrl;
    }

    private Issue issue(JsonNode node) {
        return new Issue(
                node.path("node_id").asText(node.path("id").asText()),
    ...
    private Comment comment(JsonNode node) {
        return new Comment(
                node.path("node_id").asText(node.path("id").asText()),
    ...
    private PullRequest pullRequest(JsonNode node) {

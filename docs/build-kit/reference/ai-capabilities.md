...
|------------|-----------|------------------------|-----------|-------------------|-----------------------|
| AI capture | M08 | `/v1/ai/capture`, `/v1/ai/capture/accept`, `/v1/ai/capture/reject` | Convert free-form text into structured task drafts, confidence, warnings, and optional clarification. | Core authorizes the user, validates drafts, persists accepted task data through the task application service, and records accept/reject funnel events. | Drafts must include enough task fields for review; low-confidence fields must be visible to the user. |

...
| `AWS_REGION`                 | Deploy and infra plan | AWS region for ECR, ECS, S3, CloudFront invalidation calls, and OpenTofu. Use `ap-southeast-2` for the Sydney application/data-plane environments.                                                              |
...
## Region and certificate requirements

Set the `AWS_REGION` GitHub Environment variable to `ap-southeast-2` for TaskMind
Sydney staging and production so deployments, ECR pushes, ECS service updates, S3
uploads, and OpenTofu plans target the application/data-plane region.
...
response; this fallback is intended for local/non-production composition only. CloudFront
viewer certificates are a separate edge concern: when a custom domain is attached to
CloudFront, provision or import the viewer ACM certificate in `us-east-1` as required
by CloudFront, while keeping `AWS_REGION=ap-southeast-2` for TaskMind application and
